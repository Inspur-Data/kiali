...
func (a IdleNodeAppender) AppendGraph(trafficMap graph.TrafficMap, globalInfo *graph.AppenderGlobalInfo, namespaceInfo *graph.AppenderNamespaceInfo) {
	...
	if a.GraphType != graph.GraphTypeService && a.GraphType != graph.GraphTypePod {
		workloads = getWorkloadList(namespaceInfo.Namespace, globalInfo).Workloads
	}
	if a.GraphType == graph.GraphTypePod {
		pods = getPodList(namespaceInfo.Namespace, globalInfo)
	}
	...
	a.addIdleNodes(trafficMap, globalInfo.HomeCluster, namespaceInfo.Namespace, services, workloads, pods)
}

func (a IdleNodeAppender) addIdleNodes(trafficMap graph.TrafficMap, cluster, namespace string, services []models.ServiceOverview, workloads []models.WorkloadListItem, pods models.Pods) {
	idleNodeTrafficMap := a.buildIdleNodeTrafficMap(trafficMap, cluster, namespace, services, workloads, pods)
...
func (a IdleNodeAppender) buildIdleNodeTrafficMap(trafficMap graph.TrafficMap, cluster, namespace string, services []models.ServiceOverview, workloads []models.WorkloadListItem, pods models.Pods) graph.TrafficMap {
	...
	for _, p := range pods {
		...
		id, nodeType := graph.Id(cluster, "", "", namespace, "", "", "", a.GraphType, p.Name)
		if _, found := trafficMap[id]; !found {
			if _, found = idleNodeTrafficMap[id]; !found {
				log.Tracef("Adding idle node for workload [%s] with labels [%v]", p.Name, labels)
				node := graph.NewNodeExplicit(id, cluster, namespace,"", "", "", "", nodeType, a.GraphType, p.Name)
				// note: we don't know what the protocol really should be, http is most common, it's a dead edge anyway
				node.Metadata = graph.Metadata{"httpIn": 0.0, "httpOut": 0.0, graph.IsIdle: true}
				idleNodeTrafficMap[id] = &node
			}
