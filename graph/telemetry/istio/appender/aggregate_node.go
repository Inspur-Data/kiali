...
func (a AggregateNodeAppender) appendGraph(trafficMap graph.TrafficMap, namespace string, client *prometheus.Client) {
	...
	groupBy := fmt.Sprintf(
		"source_cluster,source_workload_namespace,source_workload,source_canonical_service,source_canonical_revision,"+
			"destination_cluster,destination_service_namespace,destination_service,destination_service_name,"+
			"destination_workload_namespace,destination_workload,destination_canonical_service,destination_canonical_revision,"+
			"pod,request_protocol,response_code,grpc_response_status,response_flags,%s", a.Aggregate)
...
func (a AggregateNodeAppender) injectAggregates(trafficMap graph.TrafficMap, vector *model.Vector) {
	...
	for _, s := range *vector {
		...
		sourceID, _ := graph.Id(sourceCluster, sourceWlNs, "", sourceWlNs, sourceWl, sourceApp, sourceVer, a.GraphType, sourcePod)
		...
		if a.InjectServiceNodes {
			...
			destID, _ = graph.Id(destCluster, destSvcNs, destSvcName, destWlNs, destWl, destApp, destVer, a.GraphType, destinationPod) // wl/app
		}
