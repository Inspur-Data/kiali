...
func fetchPods(layer *Layer, namespace string, labelSelector string) (models.Pods, error) {
	var pods []core_v1.Pod
	...
	for _, p := range pods {
		pod := models.Pod{
			Name:               p.Name,
			Labels:             p.Labels,
			CreateTime:         p.CreationTimestamp,
			Status:             string(p.Status.Phase),
			StatusMessage:      p.Status.Message,
			StatusReason:       p.Status.Reason,
			ArchLabel:          p.Labels["kubernetes.io/arch"],
			Annotations:        p.Annotations,
			ServiceAccountName: p.Spec.ServiceAccountName,
		}
		if p.Annotations != nil && p.Annotations["sidecar.istio.io/inject"] == "true" {
			pod.IstioSidecar = true
		}
