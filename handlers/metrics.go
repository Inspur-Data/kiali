...
// WorkloadMetrics is the API handler to fetch metrics to be displayed, related to a single workload
func PodMetrics(w http.ResponseWriter, r *http.Request) {
	getPodMetrics(w, r, defaultPromClientSupplier)
}

// getWorkloadMetrics (mock-friendly version)
func getPodMetrics(w http.ResponseWriter, r *http.Request, promSupplier promClientSupplier) {
	vars := mux.Vars(r)
	namespace := vars["namespace"]
	pod := vars["pod"]
	...
	params := models.IstioMetricsQuery{Namespace: namespace, Pod: pod}
	err := extractIstioMetricsQueryParams(r, &params, namespaceInfo)
	if err != nil {
		RespondWithError(w, http.StatusBadRequest, err.Error())
		return
